...
Package: `@openrouter-mobile/db`. `userId` on `chats` / `generation_jobs` is `text` with FK to Better Auth `user.id`. Auth tables live in `packages/db/src/schema/auth.ts`; merge `{ ...appRelations, ...authRelations }`. Classic `createAuthDb` (postgres.js) is for Better Auth only — Effect queries still use `PgDrizzle`.

## `defineRelations` (copied from `drizzle-orm@1.0.0-rc.5-ab785fc` `relations.d.ts`)
